...
(function () {
  ...
  /* ---------- Product card (one component for home, listings, search, related) ----------
     Every product: category label, title, "Add to cart" icon button (Figma card action) and a wishlist heart.
     The cart is a quote list (no prices), so every product is addable; commerce.js owns the behaviour. */
  function cardHTML(p) {
    var c = SX.CATS[p.cat];
    ...
    return '<article class="group flex flex-col bg-white border border-bd rounded-card overflow-hidden transition-shadow hover:shadow-card">' +
      ...
      '<button type="button" data-sx-add="' + esc(p.id) + '" aria-label="Add ' + esc(p.title) + ' to cart" title="Add to cart" class="w-8 h-8 bg-[#f0eded] hover:bg-[#e6dfdc] rounded-lg flex items-center justify-center transition-colors"><img src="' + ICON + 'icon-add-cart-2.svg" alt="" width="15" height="15"></button>' +
      '</div></div></article>';
  }
